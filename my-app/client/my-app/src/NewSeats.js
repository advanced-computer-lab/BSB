...
export default function Seats() {
   ...
   const handleClose = ()=>{
    ...
    return (
        <div>
            <h1 style={{ color: ' #5c0931', marginLeft: 350 ,marginTop:"330px"}}>Choose your departure flight seats</h1>;
            <div style={{
            ...
            <Button variant="outlined" style={{ color: "white", backgroundColor: "#5c0931" ,marginLeft:"1200px", marginTop:"-800px"}} onClick={() => {
               if( chosen.length==(adult+child)){
                navigate("/newSummary", { state: { flight: flight, cab: cab, adult: adult, child: child, seats: chosen, reservation:reservation, dep:dep,  price:price, tprice:tprice, total:total  } });
                console.log(chosen);}
                else{
                    handleClick();
                }
